...
async function getCopy(description, tone, type){  
    ...
    return (type === 'product_checkbox') ? `Write a written copy content for social media for Facebook that has a ${tone} tone. \n\n Product: ${description}` : `Write a written copy content for social media for Facebook that has a ${tone} tone. \n\n Service: ${description}`;
}
...
async function getHashtag(description, tone, type){  
    ...
    // });
    // return completion.data.choices[0].text;
    return (type === 'product_checkbox') ? `Write only one completed hashtag for social media that has a ${tone} tone. \n\n Product: ${description}` : `Write a hashtag content for social media for Facebook that has a ${tone} tone. \n\n Service: ${description}`;
}


router.post("/generator/written", async function(req, res){
...
router.post("/generator/hashtag", async function(req, res){
    if(gibberish.detect(req.body.description)){
        res.json({
            statusCode: 200,
            headers:{
